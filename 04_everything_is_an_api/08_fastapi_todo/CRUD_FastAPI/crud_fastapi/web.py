...
with tab1:
    ...
    if st.button("Confirm", key='create_button'):
        res = requests.post("http://127.0.0.1:8000/todos", json={"title": todo, "description":todo_desc})
        # st.write(res.json())
...
with tab2:
    ...
    if st.button("Confirm", key='update_confirm'):
        res = requests.put(f"http://127.0.0.1:8000/todos/{idVal}", json={"title": todo, "description":todo_desc})
        # st.write(res.json())
